...
function initResetField() {
	jQuery(document).on('click', 'button.reset', function(e) {
		e.preventDefault();

		var $btn = jQuery(this);
		var $parent = $btn.closest('.wrapp-input-control, .wrapp-textarea-control');
		var $field = $parent.find('input, textarea');

		$field.val('').trigger('reset');
...
function initCountCharacters() {
	jQuery(document).on('input change blur reset', 'input[maxlength], textarea[maxlength]', function(e) {
		var $field = jQuery(this);
		var $parent = $field.closest('.wrapp-input-control, .wrapp-textarea-control');
		var $currChar = $parent.find('.js-current-characters');
		var maxLength = $field.prop('maxlength');
		var currLength = $field.val().length;

        if (currLength >= maxLength) {
        	$field.val($field.val().substring(0, maxLength));
        }

		$currChar.text(currLength);
	});
}
...
function initFormValidation() {
	jQuery('.form-validation').formValidation({
		...
		successClass: 'succes-input',
		addClassToParent: '.wrapp-input-control, .wrapp-textarea-control',
	});
}

// add class when element is in focus
function initFocusClass() {
	jQuery('.wrapp-input-control, .wrapp-textarea-control ').addFocusClass({
